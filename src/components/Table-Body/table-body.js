...
function TableBody(props) {
  ...
  return (
    <ul className="table-line" data-id={"index"}>
      ...
      <li
        className={`line-items column-value ${
          document.type === "debit" ? "orange" : "purple"
        }`}
      >
        {String((document.value / 100).toFixed(2)).replace(".", ",")}
      </li>
      <li className="line-items btn-edit-delete">
        ...
        <div className={`container-confirm-delete ${hidden}`}>
          ...
          <div>
            <button
              onClick={() => handleDelete("", "DELETE", document.id)}
            ...
            <button
              ...
              className={`btn-actions-cancel-delete`}
            >
              Não
            </button>
          </div>
