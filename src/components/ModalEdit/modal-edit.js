...
function ModalEdit(props) {
  const { handleRegisterDocument, modalEdit, closeModalEdit } = props;
  ...
  const body = {
    ...
    description:
      description &&
      description[0].toUpperCase() + description.substr(1).toLowerCase(),
    value: Number(value.substr(2).trim().replace(",", "").replace(".", "")),
    category:
      category && category[0].toUpperCase() + category.substr(1).toLowerCase(),
  ...
  function handleConfirm() {
    handleRegisterDocument(body, "PUT");
    handleClearValues();
    closeModalEdit("");
  }
  return (
    <div class={`modal ${modalEdit}`}>
      <div className="modal-container">
        <button onClick={() => closeModalEdit("")} className="close-icon">
          <img onClick={handleClearValues} src={btnClose} alt="Botão fechar" />
        </button>
        ...
        <div className="btns">
          <button
            onClick={handleBtnCredit}
            className={`credit-button ${btnCrebit}`}
          >
            Entrada
          </button>
          <button
            onClick={handleBtnDedit}
            className={`debit-button ${btnDebit}`}
          >
            Saida
          </button>
        </div>
        ...
        <button onClick={(e) => handleConfirm()} className="btn-insert">
          Confirmar
        </button>
      </div>
    </div>
