  ...
  async function listDocuments() {
    try {
      const api = await fetch(
        `https://dindin-api-desafioback.herokuapp.com/transactions`,
        {
          method: "GET",
        }
      );
  ...
  async function handleRegisterDocument(body, method, id) {
    ...
    try {
      ...
      if (method === "PUT") {
        ...
        url = `https://dindin-api-desafioback.herokuapp.com/transactions/${Number(
          id
        )}`;
      } else {
        return;
      ...
      if (filter.daysFilter.length !== 0) {
        filter.daysFilter.map((e) => {
          return data.map((element) => {
            return (
              element.week_day.toLowerCase() === e.toLowerCase() &&
      ...
      if (filter.categoryFilter.length !== 0) {
        ...
        filter.categoryFilter.map((e) => {
          return newDocuments.map((element) => {
            return (
              element.category.toLowerCase() === e.toLowerCase() &&
              arr.push(element)
            );
          });
        });
    ...
    if (type === "day" && order === "growing") {
      ...
      days.map((e) => {
        return documents.filter(
          (element) => element.week_day === e && newArrDays.push(element)
        );
      });
